...
function removeProperties(obj, keys) {
  const objWithoutKeys = shallowCopy(obj);
  keys.forEach((key) => {
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    const currentCach = Object.entries(cash).reduce((acc, [key, value]) => {
      if (value > 0) {
        return acc + key * value;
      }
      return acc;
    }, 0);
    if (queue[i] !== 25 && currentCach + 25 < queue[i]) {
...
function Rectangle(width, height) {
  ...
  return rectangleMaker(width, height);
}
...
function sortCitiesArray(arr) {
  ...
  const sortedByCountryAndCity = sortedByCountry.sort((a, b) => {
    if (a.country === b.country) {
      return a.city < b.city ? -1 : 1;
    }
    return 0;
  });
  return sortedByCountryAndCity;
}
